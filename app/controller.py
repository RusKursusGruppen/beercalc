...
def run():
    ...
    product1.add_purchase(Purchase(u"Guld Tuborg", 20000, 45))
    product1.add_purchase(Purchase(u"Harboe Guld", 7000, 35))
    inventory.add_product(product1)
    product2 = Product(u"Sodavand")
    product2.add_purchase(Purchase(u"Hindbærbrus", 7000, 45))
    product2.add_purchase(Purchase(u"Cola", 7000, 35))
    inventory.add_product(product2)

    product1.update_stock(0)

    for account in accounts.list_by_name():
        usage.update(account.id, product1.id, randint(5,5))
        usage.update(account.id, product2.id, randint(5,5))
